...
string ForMid::genCode(){
    stringstream ss;
    string forLabel = getNewLabel("for");
    string endForLabel = getNewLabel("endFor");
    Code leftSideCode;
    this->expr->genCode(leftSideCode);
    cout << leftSideCode.code;
    releaseRegister(leftSideCode.place);
     
    ss << forLabel << ": "<< endl;
    ss << leftSideCode.code << endl;
    ss << "beqz " << leftSideCode.place << ", " << endForLabel<< endl;
    ss<< this->stmt->genCode() <<endl;
    ss << "j " << forLabel << endl;
    ss << endForLabel << ": " << endl;
    return ss.str();
...
string ForStatement::genCode(){
    ...
    string forLabel = getNewLabel("for");
    string endForLabel = getNewLabel("endFor");
    string name = ((IdExpr *)this->leftExpr)->id;
    Code leftSideCode;
    Code midSideCode;
    Code rightSideCode;
    this->leftExpr->genCode(leftSideCode);
    this->midExpr->genCode(midSideCode);
    this->rightExpr->genCode(rightSideCode);
    releaseRegister(leftSideCode.place);
    
    
    ss << forLabel << ": "<< endl;
    ss << midSideCode.code << endl;
    ss << "beqz " << midSideCode.place << ", " << endForLabel<< endl;
    ss << "addi " <<leftSideCode.place<< ", " <<leftSideCode.place << ", 1"<<endl;
...
string IfStatementExtended::genCode(){
    ...
    Code exprCode1;
    Code exprCode2;
    this->expr1->genCode(exprCode1);
    this->expr2->genCode(exprCode2);
    stringstream code;
    code << exprCode1.code << endl;
    if(exprCode1.type == INT){
        code<< "beqz "<< exprCode1.place << ", " << endIfLabel <<endl;
    }else{
        code << "bc1f "<< endIfLabel <<endl;
    }
    code << endIfLabel <<":"<< endl;
    releaseRegister(exprCode1.place);

    string endIfLabel2 = getNewLabel("endif");
    code << exprCode2.code << endl;
    if(exprCode2.type == INT){
        code<< "beqz "<< exprCode2.place << ", " << endIfLabel2 <<endl;
    }else{
        code << "bc1f "<< endIfLabel2 <<endl;
    }
    code<< this->trueStatement->genCode() << endl
    << endIfLabel2 <<":"<< endl;
    releaseRegister(exprCode2.place);
...
void GteExpr::genCode(Code &code){
    ...
    if (leftSideCode.type == INT && rightSideCode.type == INT)
    {
        ...
        releaseRegister(rightSideCode.place);
        string temp = getIntTemp();
        ss<< "sle "<< temp<< ", "<< rightSideCode.place<< ", "<< leftSideCode.place<<endl;
        code.place = temp;
    }else{
        code.type = FLOAT;
        toFloat(leftSideCode);
...
void PwrAssignExpr::genCode(Code &code){
    ...
    string pwrLbl = getNewLabel("forPwr");
    string endPwrLbl = getNewLabel("endForPwr");
    string temp = "$t9";
    ss<< leftSideCode.code <<endl;
    ss<< rightSideCode.code <<endl;
    ss<< "li $t2, 1" << endl;
    ss<< pwrLbl<<":"<<endl;
    ss<< "lw "<<temp<< ", " << codeGenerationVars[name]->offset<<"($sp)"<<endl;
    ss<< "slt $t3, $t2, "<<rightSideCode.place<<endl;
    ss<< "beq $t3, $zero, "<<endPwrLbl<<endl;
    cout << leftSideCode.code << endl;
    cout << rightSideCode.code << endl;
    if(leftSideCode.type == INT){
        ss<< "mul "<< temp<< ", "<<temp<<", " << leftSideCode.place<<endl;
        //ss<< "mflo "<< leftSideCode.place<<endl;
    }else if(leftSideCode.type == FLOAT)
        ss << "mul.s "<< leftSideCode.place<<", "<< leftSideCode.place <<", "<< leftSideCode.place<<endl;
    ss<< "sw "<<temp<< ", " << codeGenerationVars[name]->offset<<"($sp)"<<endl;
...
void AndAssignExpr::genCode(Code &code){
    ...
    ss<< leftSideCode.code <<endl;
    ss<< rightSideCode.code <<endl;
    ss << "and "<<leftSideCode.place << ", " << leftSideCode.place << ", " << rightSideCode.place << endl;
...
void OrAssignExpr::genCode(Code &code){
    ...
    ss<< leftSideCode.code <<endl;
    ss<< rightSideCode.code <<endl;
    ss << "or "<<leftSideCode.place << ", " << leftSideCode.place << ", " << rightSideCode.place << endl;
...
IMPLEMENT_BINARY_GET_TYPE(AndAssign);
IMPLEMENT_BINARY_GET_TYPE(OrAssign);
...
IMPLEMENT_BINARY_ARIT_GEN_CODE(Pwr, '^');
